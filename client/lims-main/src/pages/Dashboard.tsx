...
import { 
  Library, BookOpen, CheckCircle2, AlertCircle, Users, 
  ArrowRight, Activity
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      // Students don't call the reports endpoint at all
      if (!isStaff) {
        setIsLoading(false);
        return;
      }
      try {
        ...
        setStats(response.data.data);  // ← also fix the wrapper issue
      } catch (error) {
  ...
  const staffKpis = [
    ...
    ...(isAdmin ? [{ label: 'Total Users', value: stats?.totalUsers || 0, icon: Users, color: 'primary-container', trend: '0.8%' }] : []),
  ];

  // Student KPIs — personal stats only (no sensitive library data)
  const studentKpis = [
    { label: 'My Active Loans', value: 0, icon: BookOpen, color: 'secondary', trend: '' },
    { label: 'Books Available', value: stats?.availableBooks || 0, icon: CheckCircle2, color: 'teal-500', trend: '' },
    { label: 'My Fines', value: 0, icon: AlertCircle, color: 'error', trend: '' },
  ];

  const kpis = isStaff ? staffKpis : studentKpis;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex items-end justify-between">
        <div className="space-y-1">
          {/* Personalised greeting based on role */}
          <h1 className="text-2xl font-bold text-slate-900">
            {isStaff ? 'Library Overview' : `Welcome back, ${user?.fullName?.split(' ')[0]}`}
          </h1>
          <p className="text-sm text-slate-500">
            {isStaff
              ? `Real-time activity and collection statistics for ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`
              : 'Your library activity and reading progress'}
          </p>
        </div>
        {isStaff && (
          <div className="flex gap-2">
            <div className="px-3 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded uppercase tracking-wider">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-xl p-6 text-white shadow-lg shadow-blue-200/50 relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-blue-100 text-sm leading-relaxed mb-4">
                  The Auto-Renew feature has increased student satisfaction by 14%. Consider suggesting it during checkout.
                </p>
      ...
      {!isStaff && (
        <div className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm text-center">
          <BookOpen className="mx-auto h-10 w-10 text-blue-500 mb-3" />
          <h3 className="font-bold text-slate-800 mb-1">Explore the Library</h3>
          <p className="text-slate-500 text-sm mb-4">Browse available books, manage your loans, and track your reading.</p>
          <a href="/discovery" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors">
            Discover Books <ArrowRight className="h-4 w-4" />
          </a>
        </div>
      )}
    </div>

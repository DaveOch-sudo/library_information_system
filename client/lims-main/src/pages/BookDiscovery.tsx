...
import { Search, Grid, List as ListIcon, Star, ArrowRight, Bookmark, ArrowUpRight, BookOpen } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="relative bg-slate-900 text-white p-10 md:p-14 rounded-2xl overflow-hidden shadow-xl">
        <div className="relative z-10 max-w-2xl space-y-6">
          <div className="space-y-2">
            <span className="text-blue-400 font-bold text-[10px] tracking-[0.2em] uppercase">Discovery Engine</span>
            <h1 className="text-3xl md:text-5xl font-bold tracking-tight leading-[1.1]">Explore our vast academic collection.</h1>
            <p className="text-slate-400 text-sm md:text-base max-w-md">Search across 12,000+ volumes, periodicals, and digital resources.</p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-2 rounded-xl border border-slate-200 gap-4">
        <div className="flex p-1 bg-slate-50 rounded-lg w-full sm:w-auto">
          {['Recommended', 'Most Popular', 'Recently Added'].map((tab, i) => (
            <button key={i} className={cn(
              ...
              i === 0 ? "bg-white text-primary shadow-sm" : "text-slate-500 hover:text-slate-700"
            )}>
              {tab}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2 px-2">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-2">Layout</span>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1,2,3,4,5,6,7,8].map(i => <div key={i} className="h-96 bg-slate-100 animate-pulse rounded-2xl"></div>)}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredBooks.map((book) => (
            <div key={book.id} className="group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col hover:-translate-y-1">
              <div className="h-52 bg-slate-50 relative overflow-hidden flex items-center justify-center group-hover:bg-slate-100 transition-colors">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  <div className="bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded-md text-[10px] font-bold text-slate-700 flex items-center shadow-sm border border-slate-100">
                     <Star className="h-3 w-3 text-amber-500 fill-current mr-1" /> 4.9
                  </div>
                </div>

                {book.availableCopies === 0 && (
                   <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-[1px] flex items-center justify-center p-4">
                      <span className="bg-white text-red-600 font-bold px-3 py-1.5 rounded-md text-[10px] uppercase shadow-xl tracking-wider">Out of Stock</span>
                   </div>
                )}
              ...
              <div className="p-5 flex-1 flex flex-col space-y-3">
                <div className="space-y-1">
                  <span className="text-blue-600 font-bold text-[9px] uppercase tracking-widest">{book.category.name}</span>
                  <h4 className="text-base font-bold text-slate-900 line-clamp-2 leading-snug group-hover:text-primary transition-colors">{book.title}</h4>
                  <p className="text-slate-500 text-xs">
                    <span className="text-slate-400">by</span> {book.author.name}
                  </p>
                </div>
                
                <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                  <div className="flex flex-col">
                     <span className="text-[9px] text-slate-400 uppercase font-bold tracking-tighter">Availability</span>
                     <span className="text-sm font-bold text-slate-900">{book.availableCopies} Copies</span>
                  </div>
                  <button 
                    onClick={() => handleBorrow(book.id)}
                    disabled={book.availableCopies === 0}
                    className="flex items-center gap-1.5 text-xs font-bold text-primary px-3 py-1.5 bg-blue-50/50 hover:bg-blue-50 rounded-lg transition-colors border border-blue-100/50 disabled:opacity-30 disabled:grayscale"
                  >
